 ...
 * architectures whose backend can describe per-event scheduling
 * constraints (x86/AMD today; see amd_get_sched_constraint()).  There
 * hwpmc_pmu.c and hwpmc_assign.c are compiled in and export the real
 * entry points below.  Everywhere else the architecture-independent
 * hwpmc_mod.c is linked against the no-op stubs in the #else branch, so
 * the module builds and behaves exactly as it did before the grouping
 * layer was added.
 */
#if defined(__amd64__) || defined(__i386__)
#define	HWPMC_PMU_GROUPS	1
#endif

#ifdef HWPMC_PMU_GROUPS
...
#else /* !HWPMC_PMU_GROUPS */

/*
 * No-op stubs for architectures that do not compile in the PMU grouping
 * layer.  Userland never sets the PMC_F_GROUP_* flags or issues the
 * group ioctls on such kernels, so allocate/create paths fail cleanly
 * with EOPNOTSUPP and the context-switch / release hooks are nothing
 * but cheap no-ops -- behaviour is identical to a pre-grouping hwpmc.
 */
static __inline int
pmu_group_on_allocate(struct pmc *pm __unused,
    const struct pmc_op_pmcallocate *pa __unused)
{
	return (EOPNOTSUPP);
}

static __inline int
pmu_group_on_start(struct pmc *pm __unused)
{
	return (0);
}

static __inline void
pmu_group_on_stop(struct pmc *pm __unused)
{
}

static __inline void
pmu_group_on_release(struct pmc *pm __unused)
{
}
...
static __inline int
pmu_group_create(struct pmc_owner *po __unused, uint32_t *pg_id __unused)
{
	return (EOPNOTSUPP);
}

static __inline int
pmu_group_add(pmu_group_t *pg __unused, struct pmc *pm __unused,
    bool leader __unused)
{
	return (EOPNOTSUPP);
}

static __inline int
pmu_group_commit(pmu_group_t *pg __unused)
{
	return (EOPNOTSUPP);
}

static __inline pmu_group_t *
pmu_group_lookup(struct pmc_owner *po __unused, uint32_t pg_id __unused)
{
	return (NULL);
}
